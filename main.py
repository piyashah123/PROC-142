...
@app.route("/recommended_movie_data")
def recommended_movies():
    global liked_movie
    col_names=['original_title','poster_link','release_date','runtime','wigthed_rating']
    all_recommended=pd.DataFrame(columns=col_names)
    for liked_movie in liked_movies:
        output=get_recommendations(liked_movie=["original_title"],inplace=True)
        all_recommended=all_recommended.append(output)
    all_recommended.drop_duplicates(subset=['original_title'],inplce=True)
